...
export default function BunkerModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg card-panel border border-gold/30 rounded-3xl p-6 max-h-[88vh] overflow-y-auto"
      ...
      >
        ...
        <div className="rounded-2xl bg-gold/[0.06] border border-gold/20 p-4 mb-4">
          ...
          {BUNKER_AUDIO_URL ? (
            <button
              onClick={toggle}
              className="mt-3 flex items-center gap-2 px-4 py-2 rounded-lg bg-gold/15 border border-gold/30 text-gold text-xs font-bold uppercase tracking-wider"
            >
            ...
            <ul className="space-y-1.5">
              {compromisos.slice(0, 5).map((c, i) => (
                <li key={i} className="flex items-start gap-2 text-sm text-cream/75">
                  <Flame className="w-3.5 h-3.5 text-gold mt-0.5 shrink-0" />
                  {c}
                </li>
              ))}
            </ul>
        ...
        <button
          onClick={onClose}
          className="w-full py-3 rounded-xl bg-gold text-ink text-sm font-bold uppercase tracking-wider hover:bg-goldhi transition-colors"
        >
          Me quedo un día más 🥋
        </button>
      </div>
