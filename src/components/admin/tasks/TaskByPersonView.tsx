            ...
            return (
              <div key={bucket} className="space-y-2">
                <div className="flex items-center justify-between px-2">
                  <span className={`text-sm font-bold uppercase tracking-wider ${isVencidas ? 'text-red-400' : 'text-cream/55'}`}>
                    {BUCKET_LABELS[bucket]}
                  </span>
                  <span className={`text-sm font-bold px-1.5 py-0.5 rounded-full ${isVencidas ? 'bg-red-500/15 text-red-400' : 'bg-cream/5 text-cream/55'}`}>
                    {items.length}
                  </span>
                </div>
