...
const LABEL_CLASS = 'block text-sm font-bold text-cream/55 uppercase tracking-wider mb-1.5';
      ...
      <div className="bg-panel border border-gold/12 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gold/10 flex-shrink-0">
          <div>
            ...
            <p className="text-sm text-cream/55 mt-0.5">Paso {step + 1} de {STEPS.length} — {STEPS[step]}</p>
          </div>
          ...
          {STEPS.map((label, i) => (
            <div key={i} className="flex items-center flex-1">
              <div className={`flex items-center gap-1.5 ${i <= step ? 'text-gold' : 'text-cream/20'}`}>
                <div className={`w-5 h-5 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${i < step ? 'bg-gold text-black' : i === step ? 'border-2 border-gold text-gold' : 'border border-cream/20 text-cream/20'}`}>
                  {i < step ? <Check className="w-3 h-3" /> : i + 1}
                </div>
                <span className="text-sm font-semibold hidden sm:block whitespace-nowrap">{label}</span>
              </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-3">
            <label className="block text-sm text-cream/75 mb-1">Avatar del sanador (para el Mentor)</label>
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-sm text-cream/45 mt-1">{textoLibre.length} caracteres</p>
              </div>
                  ...
                  return (
                    <div key={fieldKey}>
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        {isAi && (
                          <span className="text-sm font-bold px-1.5 py-0.5 rounded-full bg-gold/15 text-gold">IA</span>
                        )}
                      </div>
                      <textarea
