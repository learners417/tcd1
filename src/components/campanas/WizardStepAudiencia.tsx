      ...
      <div>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-black/20 border border-[rgba(232,150,46,0.12)] rounded-lg px-4 py-2.5 text-cream placeholder:text-cream/25 focus:outline-none focus:border-gold/50"
          />
          <button
            onClick={addInteres}
            className="px-3 py-2.5 bg-gold/10 border border-gold/30 rounded-lg text-gold hover:bg-gold/20 transition-colors"
          >
        ...
        {form.intereses.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-2">
            {form.intereses.map((interes, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gold/10 text-gold text-xs font-medium border border-gold/20"
              >
                {interes}
                <button onClick={() => removeInteres(idx)} className="hover:text-cream transition-colors">
                  <X className="w-3 h-3" />
                </button>
              </span>
            ))}
