...
export const SESIONES_GUIADAS: Record<string, SesionGuiada> = {
  ...
  'P0.3': {
    ...
    pasoEsencialIdx: 2,
    pasos: [
      { tipo: 'intro', titulo: 'El Búnker de Luz', texto: 'Este camino tiene noches oscuras — el día que un {{consultante}} no llega, el día que el miedo grita. Para esas noches se construye HOY tu búnker: una caja física con los objetos que te devuelven a quién eres. Tu tanque de oxígeno. Se arma antes de necesitarlo — nunca durante la tormenta.', nota: 'Busca una caja real. Cualquiera sirve: de zapatos, de madera, de lata.' },
      { tipo: 'opciones', id: 'objetos', pregunta: '¿Qué va a vivir en tu búnker? Elige lo que te carga de verdad:', opciones: ['Una foto de alguien que amo', 'Una carta o mensaje que me marcó', 'Un objeto de un logro mío', 'Algo que me regaló un {{consultante}} agradecido', 'Un símbolo de mi fe', 'Algo de alguien que ya no está'], multiple: true, permiteOtra: true },
  ...
  'P1.2': {
    ...
    pasoEsencialIdx: 4,
    pasos: [
      { tipo: 'intro', titulo: 'Tu radiografía', texto: 'Hoy no hay que convencer a nadie. Hay que VER. Como una radiografía de verdad: no juzga — muestra. Una sola condición, la del pacto: la verdad verdad, no la verdad bonita. Una radiografía maquillada no le sirve a ningún médico… y hoy el {{consultante}} eres tú.' },
      ...
      { tipo: 'opciones', id: 'cuerpo', pregunta: 'Cuando un {{consultante}} te pregunta «¿cuánto cuesta?»… ¿qué pasa en tu cuerpo?', opciones: ['Se me cierra la garganta', 'Se me aprieta el pecho o el estómago', 'Empiezo a explicar de más', 'Bajo el precio antes de que objeten', 'Lo digo tranquilo — sin problema'], permiteOtra: true },
  ...
  /* D11 · {{Consultante}} ideal — LA MATRIZ ABC (G5) */
  'P2.3': { codigo: 'P2.3', pasoEsencialIdx: 3, pasos: [
    { tipo: 'intro', titulo: '{{Tu_consultante}} ideal — la matriz ABC', texto: 'Un restaurante donde cada uno pide lo que se le antoja no es un negocio. Hoy eliges QUIÉN se sienta en tu mesa. La matriz es simple: {{consultantes}} A (los amas, pagan bien, avanzan), B (correctos), C (te drenan, regatean, no avanzan). Tu clínica nueva se construye para los A.' },
    { tipo: 'abierta', id: 'mejor_a', pregunta: 'Tu mejor {{consultante}} A — el que llegó roto y salió entero. ¿Quién era, qué le dolía, y por qué trabajar con esa persona te llenaba?', ayuda: 'Cuéntalo hablando: toca el micrófono. Si no tienes 3 {{consultantes}} aún, cuenta TU propia historia — tú fuiste tu primer caso.', minChars: 120, placeholder: 'Mi mejor {{consultante}}…' },
    ...
    { tipo: 'artefacto', id: 'ideal', tituloArtefacto: 'Tu {{Consultante}} Ideal (la matriz ABC)', promptIA: 'Con su mejor caso A, las señales A y las señales C, redacta "Tu {{Consultante}} Ideal": (1) retrato del A — quién es, qué le duele, qué compra de verdad (2) las 3 señales de entrada de un A (3) las señales de un C y la regla: al C se lo deriva con amor — «los veganos, a la cuadra de enfrente» (4) la frase de posicionamiento: para quién ES tu clínica. Máx 170 palabras, segunda persona.', campoAdn: 'paciente_ideal' },
  ]},
  ...
  'P4.2b': { codigo: 'P4.2b', pasoEsencialIdx: 3, pasos: [
    { tipo: 'intro', titulo: 'Tu perfil que convierte', texto: 'Tu perfil es tu recepción. El {{consultante}} te googlea ANTES de escribirte — y decide en 5 segundos. Hoy lo dejamos profesional: foto, bio, destacadas. Reglas de oro: la foto es TU ROSTRO llenando el círculo (el ojo humano identifica rostros, no cuerpos lejanos — pruébala en chiquitito), y la bio dice la TRANSFORMACIÓN, no los títulos.' },
    ...
    { tipo: 'ia_propone', id: 'bio', pregunta: 'Tu bio nueva — transformación, no currículum:', promptIA: 'Con su especialidad, método con nombre y {{consultante}} ideal, propone 3 bios de Instagram (máx 150 caracteres c/u): línea 1 = a quién ayuda y a qué resultado · línea 2 = el método con nombre · línea 3 = llamado simple. Sin «digital creator», sin títulos académicos apilados, sin emojis excesivos.', nOpciones: 3 },
  ...
  'P4.3': { codigo: 'P4.3', pasoEsencialIdx: 3, pasos: [
    ...
    { tipo: 'ia_propone', id: 'errores', pregunta: 'Los 3 errores de tu campo que tus piezas van a desarmar:', promptIA: 'Con su especialidad, método y {{consultante}} ideal, propone 5 errores comunes que su {{consultante}} ideal comete (o cree correctos) y que su método desarma. Formato: «No sigas [error]» — máx 12 palabras cada uno, específicos del campo, con filo (que frenen el scroll).', nOpciones: 5 },
  ...
  'P6.3': { codigo: 'P6.3', pasoEsencialIdx: 0, pasos: [
    { tipo: 'intro', titulo: '💰 El fruto maduro', texto: 'Si estás acá, pasó — o está por pasar hoy. El primer {{consultante}} de $1.000. Sube el comprobante: este es EL cinturón rojo, el que nadie te regaló.' },
    ...
    { tipo: 'artefacto', id: 'primer_pago', tituloArtefacto: 'El acta del primer {{consultante}}', promptIA: 'Con su relato del momento, redacta "El acta del primer {{consultante}}" (máx 120 palabras), solemne y emocionada sin cursilería: la fecha queda grabada · lo que ese pago PRUEBA (el método funciona, el precio es digno, el sistema vende) · el pacto de reinversión de su Plan se activa ahora · y el cierre: el primero es el más difícil — los otros nueve ya saben el camino.', campoAdn: 'primer_pago' },
  ]},
  ...
  'P7.2': { codigo: 'P7.2', pasoEsencialIdx: 2, pasos: [
    ...
    { tipo: 'opciones', id: 'ritmo', pregunta: 'Tu semana tipo de cazador — elige tu estructura:', opciones: ['Llamadas concentradas: 2 bloques fijos (ej. mar/jue tarde)', 'Llamadas distribuidas: 1 por día en hueco fijo', 'Según agenda de {{consultantes}} — bloques móviles cada domingo'] },
