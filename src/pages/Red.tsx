  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="card-panel p-6">
        ...
        {/* Premio */}
        <div className="rounded-xl bg-gold/[0.06] border border-gold/20 p-3 mb-4 flex items-start gap-2">
          <Sparkles className="w-4 h-4 text-gold mt-0.5 shrink-0" />
          <p className="text-xs text-cream/75 leading-relaxed">
            Por cada fundador que entra con tu llave, ganás <span className="text-gold font-medium">{PREMIO_TEXTO}</span>.
            {redimidas > 0 ? (
        ...
        {avisoInv ? <p className="text-[11px] text-gold mb-3">{avisoInv}</p> : null}

        <button
          onClick={generar}
          disabled={generando || restantes <= 0}
          className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-gold text-ink text-sm font-bold uppercase tracking-wider hover:bg-goldhi transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
      ...
      <div className="card-panel p-6">
        ...
        <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4 mb-5">
          ...
          <textarea
            ...
            className="w-full bg-[#0F0F0F] border border-white/10 rounded-lg px-3 py-2.5 text-sm text-cream placeholder-white/25 focus:border-gold/50 focus:outline-none resize-none mb-2"
          />
          {tipo === 'objecion' ? (
            <textarea
              value={respuesta}
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gold/15 border border-gold/30 text-gold text-xs font-bold uppercase tracking-wider hover:bg-gold/25 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
          ...
          <div className="space-y-3">
            {herencia.map((h, i) => (
              <div key={h.id ?? i} className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4">
                ...
                <p className="text-sm text-cream/90 leading-relaxed">{h.texto}</p>
                {h.respuesta ? (
                  <div className="mt-2 flex items-start gap-2 rounded-lg bg-success/[0.06] border border-success/15 px-3 py-2">
                    <Flame className="w-3.5 h-3.5 text-success mt-0.5 shrink-0" />
                    <p className="text-xs text-cream/75 leading-relaxed">{h.respuesta}</p>
                  </div>
                ) : null}
              </div>
            ))}
